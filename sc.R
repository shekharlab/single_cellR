...
NormalizeData <- function(
  ...
  if (normalization.method == "logTPM") {
    ...
    if (pseudocount.use != 1){
      ...
      colnames(normalized.data) = colnames(data);
      rownames(normalized.data) = rownames(data)
      rm(data)
    }
  ...
  Ligerize = function(object,batch_id="batch", var.genes = NULL,  do.clustering=TRUE, max.pcs=90, do.umap=FALSE, ...){
    ...
    #tsplot(object)
    object@tsne.rot = object@data.info[,1:2]
    colnames(object@tsne.rot) = c("tsne_1", "tsne_2")
    ...
    eval(expr = parse(text = paste0("object@dr$", "tsne", "<- tsne.obj")))
